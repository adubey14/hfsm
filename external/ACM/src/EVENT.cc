...
namespace ACM_INTERNAL{
	...
	APEX_EVENT_TYPE::APEX_EVENT_TYPE(std::string name,MUTEX* useMutex, bool locked , MUTEX::Attribute attr  ): EVENT_STATE(DOWN), 
		EVENT_NAME(name),
		APEX_UNSIGNED_COUNTER(0),																			
		APEX_UNSIGNED_COUNTER_MUTEX(false,MUTEX::NORMAL),
		CreatedMutex(false)
	{
		if(useMutex==0)
		{
	...
	std::string APEX_EVENT_TYPE::GET_NAME(){
		return this->EVENT_NAME;
	}

	APEX_EVENT_TYPE* APEX_EVENT_TYPE::CREATE_EVENT(std::string name,MUTEX* useMutex, bool locked  , MUTEX::Attribute attr  ){
	...
	void APEX_EVENT_TYPE::SET_EVENT (RETURN_CODE_TYPE *RETURN_CODE )
	{
		...
		{
			this->EVENT_STATE=UP;
			this->EVENT_CONDITION_VARIABLE_MUTEX->try_lock(); //if its already locked and is Normal, it will not lock
			//LOGGER(ACM_USER::DEBUG,"Locked Mutex for Event %s",EVENT_NAME.c_str());
			this->EVENT_CONDITION_VARIABLE->broadcast();
			//LOGGER(ACM_USER::DEBUG,"Broadcast message sent for Event %s",EVENT_NAME.c_str());
	...
	void APEX_EVENT_TYPE::WAIT_EVENT(SYSTEM_TIME_TYPE TIME_OUT,RETURN_CODE_TYPE *RETURN_CODE)
	{
		...
		while(this->EVENT_STATE==DOWN){		
			result = this->EVENT_CONDITION_VARIABLE->timed_wait(timeout);//wait until timeout
			if (result==ETIMEDOUT){
				break;
			}			
		}
	...
	void APEX_EVENT_TYPE::WAIT_EVENT(SYSTEM_TIMESPEC timeout,RETURN_CODE_TYPE *RETURN_CODE)
	{
	...
	void APEX_EVENT_TYPE::WAIT_AND_LOCK_EVENT(SYSTEM_TIMESPEC TIME_OUT,RETURN_CODE_TYPE *RETURN_CODE)
	{
		//when you enter this method process_table mutex should be unlocked;
	//	assert(this->EVENT_CONDITION_VARIABLE_MUTEX->try_lock_without_locking() == MUTEX::NO_ERROR);
		int result =0;
		//AUTO_LOCK another_Lock (*this->EVENT_CONDITION_VARIABLE_MUTEX);
		//<!Lock
		...
		//<!UNlock and Lock
		while(this->EVENT_STATE==DOWN){		
			result = this->EVENT_CONDITION_VARIABLE->timed_wait(TIME_OUT);//wait until timeout
			if (result==ETIMEDOUT){
				this->EVENT_CONDITION_VARIABLE_MUTEX->lock(); 
				break;
			}			
	...
	//	assert(this->EVENT_CONDITION_VARIABLE_MUTEX->try_lock_without_locking() == MUTEX::ALREADY_LOCKED);
	...
	EVENT_STATE_TYPE APEX_EVENT_TYPE::GET_STATE(){
